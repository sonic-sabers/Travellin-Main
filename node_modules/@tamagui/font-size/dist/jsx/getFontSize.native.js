...
const getFontSize = (inSize, opts) => {
  ...
  return token ? getConfig().fontsParsed[(opts == null ? void 0 : opts.font) || "$body"].size[token] : inSize;
}, getFontSizeToken = (inSize, opts) => {
  if (typeof inSize == "number")
    return null;
  const relativeSize = (opts == null ? void 0 : opts.relativeSize) || 0, fontSize = getConfig().fontsParsed[(opts == null ? void 0 : opts.font) || "$body"].size, size = (inSize === "$true" ? "$4" : inSize) || ("$true" in fontSize ? "$true" : "$4"), sizeTokens = Object.keys(fontSize);
